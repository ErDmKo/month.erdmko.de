...
Источник правды по контракту вынесен в отдельный файл:
- `/contracts/chat/CHAT_CONTRACT.md`

В этом плане остается только ссылка, чтобы избежать расхождений между версией плана и фактическим контрактом.
