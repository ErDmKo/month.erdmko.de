...
impl ChatWs {
    pub(super) fn on_binary(&mut self, bytes: &[u8], ctx: &mut ws::WebsocketContext<Self>) {
        if bytes.is_empty() {
            Self::send_error(&self.room_id, &self.sender_id, ctx, None, "BAD_PAYLOAD", "Empty frame.");
            return;
        }
        if !chat_service::is_valid_binary_payload_size(bytes.len()) {
            Self::send_error(&self.room_id, &self.sender_id, ctx, None, "BAD_PAYLOAD", "Payload exceeds limit.");
            return;
        }
