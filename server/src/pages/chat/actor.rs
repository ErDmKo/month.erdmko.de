...
impl ChatWs {
    fn on_text(&mut self, text: &str, ctx: &mut ws::WebsocketContext<Self>) {
        ...
        match event {
            ClientEvent::Join { request_id, nickname } => {
                self.on_join(request_id, nickname, ctx);
            }
            ClientEvent::Message { request_id, body } => {
                self.on_message(request_id, body, ctx);
            }
            ClientEvent::Delete { request_id, message_id } => {
                self.on_delete(request_id, message_id, ctx);
            }
            ClientEvent::UploadStart { request_id, message_id, filename, size, mime_type } => {
                self.on_upload_start(request_id, message_id, filename, size, mime_type, ctx);
            }
            ClientEvent::UploadEnd { request_id, upload_id } => {
                self.on_upload_end(request_id, upload_id, ctx);
            }
            ClientEvent::DownloadRequest { request_id, attachment_id } => {
                self.on_download_request(request_id, attachment_id, ctx);
            }
        }
    }

    fn on_binary(&mut self, bytes: &[u8], ctx: &mut ws::WebsocketContext<Self>) {
